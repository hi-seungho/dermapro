...
// 파일개수 체크
$file_count   = 0;
if($_FILES['cf_1_subj']['name'] && is_uploaded_file($_FILES['cf_1_subj']['tmp_name']))
    $file_count++;

// 디렉토리가 없다면 생성합니다. (퍼미션도 변경하구요.)
@mkdir(G5_DATA_PATH.'/file/config', G5_DIR_PERMISSION);
@chmod(G5_DATA_PATH.'/file/config', G5_DIR_PERMISSION);
...
if (isset($_POST['cf_1_subj_del']) && $_POST['cf_1_subj_del']) {
    $sql = " update {$g5['config_table']}
            set cf_1_subj = '',
                cf_1 = '' ";
    ...
    @unlink(G5_DATA_PATH.'/file/config/'.$_POST['cf_1']);
} else {
    $tmp_file  = $_FILES['cf_1_subj']['tmp_name'];
    $filesize  = $_FILES['cf_1_subj']['size'];
    $filename  = $_FILES['cf_1_subj']['name'];
    $filename  = get_safe_filename($filename);

// 서버에 설정된 값보다 큰파일을 업로드 한다면
    if ($filename) {
        if ($_FILES['cf_1_subj']['error'] == 1) {
            $file_upload_msg .= '\"'.$filename.'\" 파일의 용량이 서버에 설정('.$upload_max_filesize.')된 값보다 크므로 업로드 할 수 없습니다.\\n';
        }
        else if ($_FILES['cf_1_subj']['error'] != 0) {
    ...
    if (is_uploaded_file($tmp_file)) {
        ...
        $dest_file = G5_DATA_PATH.'/file/config/'.$upload['file'];

        // 업로드가 안된다면 에러메세지 출력하고 죽어버립니다.
        $error_code = move_uploaded_file($tmp_file, $dest_file) or die($_FILES['cf_1_subj']['error']);
...
// 삭제에 체크가 되어있다면 파일을 삭제합니다.
if (isset($_POST['cf_2_subj_del']) && $_POST['cf_2_subj_del']) {
    $sql = " update {$g5['config_table']}
            set cf_2_subj = '',
                cf_2 = '' ";
    ...
    @unlink(G5_DATA_PATH.'/file/config/'.$_POST['cf_2']);
} else {
    $tmp_file  = $_FILES['cf_2_subj']['tmp_name'];
    $filesize  = $_FILES['cf_2_subj']['size'];
    $filename  = $_FILES['cf_2_subj']['name'];
    $filename  = get_safe_filename($filename);

// 서버에 설정된 값보다 큰파일을 업로드 한다면
    if ($filename) {
        if ($_FILES['cf_2_subj']['error'] == 1) {
            $file_upload_msg .= '\"'.$filename.'\" 파일의 용량이 서버에 설정('.$upload_max_filesize.')된 값보다 크므로 업로드 할 수 없습니다.\\n';
        }
        else if ($_FILES['cf_2_subj']['error'] != 0) {
            $file_upload_msg .= '\"'.$filename.'\" 파일이 정상적으로 업로드 되지 않았습니다.\\n';
        }
    }
    ...
    if (is_uploaded_file($tmp_file)) {
        ...
        // 업로드가 안된다면 에러메세지 출력하고 죽어버립니다.
        $error_code = move_uploaded_file($tmp_file, $dest_file) or die($_FILES['cf_2_subj']['error']);
